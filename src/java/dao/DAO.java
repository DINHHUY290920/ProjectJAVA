...
public class DAO {
    ...
    public List<Order> getHistory2() {
        List<Order> list = new ArrayList();
        try {
            String sql = "select o.OrderID, p.ProductName, p.ProductPrice,o.OrderQuantity, o.OrderTotalPrice, o.username\n"
                    + "  from Product p, [Order] o\n"
                    +" where p.ProductID = o.productid";
            PreparedStatement ps = connection.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new Order(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getInt(5),rs.getString(6)));
